...
### 收藏分頁
- 篩選軸：顏色 / 類型 / Bloom / 稀有度 / 補充包 / 標籤 / **藝人（分組下拉）** / 持有狀態 / 關鍵字 / 排序。（HP 篩選已移除）
...
### 牌組分頁
- 規則（加入時擋）：1 推し + 50 主 + 20 應援。**每張卡上限以 card_number 計（非卡名）**，因為多張卡會共用同一藝人名。
  - 主牌組：一般卡同卡號上限 4；**Debut Holomem 例外（不限張，只受 50 總量限制）**。
  - 應援(エール)：不限同卡號張數，只受 20 總量限制。
  - 函式：`deckPerCardLimit()`、`deckCountByNumber()`、`deckSectionTotal()`、`DECK_SECTION_MAX`。
- 每個牌段的挑卡面板有**和收藏一樣的完整篩選**（可摺疊）。
- 缺卡計算 → 購物清單。
- 單一牌組匯出：**牌組碼**（可分享）+ JSON；可貼牌組碼/JSON 匯入。⚠️ 牌組碼與官方 Deck Log **不互通**，匯入/複製時有提示。
- 牌組改名是 in-place 更新側欄標籤（不重繪編輯器，避免每打一字失焦）。

### 掃描分頁（用感知雜湊，不是 OCR；UI 標示「測試中」）
- 相機 → 拍攝 → 取卡圖區算 **dHash** → 跟 `hashes.json` 算漢明距離 → 列前 6 候選 → 點選加入。
- 手機版：相機框限高 42vh、整個分頁可捲動、出結果後 `scrollIntoView`，避免相機擋住候選清單。
